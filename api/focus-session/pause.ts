...
import { pauseCurrentFocusSessionData } from '@/server/repository';
import { focusSessionPauseSchema } from '@/server/validation';
...
export default async function handler(req: any, res: any) {
  ...
  const parsed = focusSessionPauseSchema.safeParse(req.body ?? {});

  if (!parsed.success) {
    const response = jsonError('invalid_focus_session', 'Pause payload is invalid', 400);
    return res.status(response.status).json(response.body);
  }

  const data = await pauseCurrentFocusSessionData(parsed.data.remainingSeconds);
